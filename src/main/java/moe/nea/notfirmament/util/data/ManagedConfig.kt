...
import moe.nea.notfirmament.gui.config.GuiAppender
import moe.nea.notfirmament.gui.config.HudMeta
import moe.nea.notfirmament.gui.config.HudMetaHandler
import moe.nea.notfirmament.gui.config.HudPosition
...
abstract class ManagedConfig(
	...
	enum class Category {
		...
		val labelText: Component = Component.translatable("notfirmament.config.category.${name.lowercase()}")
		val description: Component = Component.translatable("notfirmament.config.category.${name.lowercase()}.description")
...
//	}
	...
	protected fun position(
		...
		val label = Component.translatable("notfirmament.config.${name}.${propertyName}")
		return option(propertyName, {
			val p = default()
			HudMeta(HudPosition(p.x(), p.y(), 1F), NotFirmament.identifier(propertyName), label, width, height)
	...
	protected fun keyBindingWithOutDefaultModifiers(
		...
		return option(propertyName, default, KeyBindingHandler("notfirmament.config.${name}.${propertyName}", this))
	}
	...
	val translationKey get() = "notfirmament.config.${name}"
