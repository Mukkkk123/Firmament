...
package moe.nea.notfirmament.init;
...
public class HandledScreenRiser extends RiserUtils {
	...
	void addKeyReleased(ClassNode classNode) {
		addSuperInjector(
			classNode, keyReleased.mapped(), keyReleased.mappedDesc(), HandledScreen, Screen, "keyReleased_notfirmament",
			insns -> {
				// ALOAD 0, load this
	...
	void addMouseClicked(ClassNode classNode) {
		addSuperInjector(
			...
			Screen, AbstractContainerEventHandler, "onMouseClicked$notfirmament",
			insns -> {
				// load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
				// load mouse event
	...
	void addCharTyped(ClassNode classNode) {
		addSuperInjector(
			...
			HandledScreen, Screen, "charTyped_notfirmament",
			insns -> {
				// ALOAD 0, load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
				// ALOAD 1, load args
	...
	void addMouseScroll(ClassNode classNode) {
		addSuperInjector(
			classNode, mouseScrolled.mapped(), mouseScrolled.mappedDesc(), HandledScreen, Screen, "mouseScrolled_notfirmament",
			insns -> {
				// ALOAD 0, load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
