...
object WarpUtil {
	...
	fun teleportToNearestWarp(island: SkyBlockIsland, pos: Position) {
		...
		if (nearestWarp == null) {
			MC.sendChat(Component.translatable("notfirmament.warp-util.no-warp-found", island.userFriendlyName))
			return
		}
		if (island == SBData.skyblockLocation
			...
			MC.sendChat(Component.translatable("notfirmament.warp-util.already-close", nearestWarp.warp))
			return
		}
		MC.sendChat(Component.translatable("notfirmament.warp-util.attempting-to-warp", nearestWarp.warp))
	...
	fun clearUnlockedWarpsCommand(event: CommandEvent.SubCommand) {
		event.subcommand("clearwarps") {
			thenExecute {
				...
				DConfig.markDirty()
				source.sendFeedback(Component.translatable("notfirmament.warp-util.clear-excluded"))
			}
		}
	}
	...
	init {
		ProcessChatEvent.subscribe("WarpUtil:processChat") {
			if (it.unformattedString == "You haven't unlocked this fast travel destination!"
				...
				DConfig.markDirty()
				MC.sendChat(Component.translatableEscape("notfirmament.warp-util.mark-excluded", lastAttemptedWarp))
