...
import moe.nea.notfirmament.apis.Routes
...
import moe.nea.notfirmament.util.boolColour
			...
			event.subcommand(DeveloperFeatures.DEVELOPER_SUBCOMMAND) {
				thenLiteral("party") {
					thenLiteral("refresh") {
						thenExecute {
							sendSyncPacket()
							source.sendFeedback(tr("notfirmament.dev.partyinfo.refresh", "Refreshing party info"))
						}
					}
					thenExecute {
						val p = party
						...
						text.append(
							tr("notfirmament.dev.partyinfo", "Party Info: ")
								.boolColour(p != null)
						)
						if (p == null) {
							text.append(tr("notfirmament.dev.partyinfo.empty", "Empty Party").grey())
						} else {
							text.append(tr("notfirmament.dev.partyinfo.count", "${p.members.size} members").grey())
							p.members.forEach {
								text.append("\n")
									...
									.append(
										when (it.role) {
											PartyRole.LEADER -> tr("notfirmament.dev.partyinfo.leader", "Leader").bold()
											PartyRole.MOD -> tr("notfirmament.dev.partyinfo.mod", "Moderator")
											PartyRole.MEMBER -> tr("notfirmament.dev.partyinfo.member", "Member")
										}
									)
									.append(")")
							}
