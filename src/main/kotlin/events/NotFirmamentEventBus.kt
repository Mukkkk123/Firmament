 ...
 */
open class NotFirmamentEventBus<T : NotFirmamentEvent> {
    companion object {
        val allEventBuses = mutableListOf<NotFirmamentEventBus<*>>()
    }

    val eventType = TypeUtils.getTypeArguments(javaClass, NotFirmamentEventBus::class.java)!!.values.single()
    ...
    fun publish(event: T): T {
        for (function in toHandle) {
            if (function.receivesCancelled || event !is NotFirmamentEvent.Cancellable || !event.cancelled) {
                try {
                    function.invocation(event)
                } catch (e: Exception) {
                    val klass = e.javaClass
                    if (!function.knownErrors.contains(klass) || NotFirmament.DEBUG) {
