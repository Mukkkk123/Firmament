	...
	fun onRenderHud(it: HudRenderEvent) {
		if (TConfig.dayCount) {
			...
			it.context.drawString(
				MC.font,
				Component.literal(String.format(tr("notfirmament.config.hud.day-count-hud.display", "Day: %s").string, day)),
				36,
				MC.font.lineHeight,
				-1,
				true
		...
		if (TConfig.fpsCount) {
			...
			it.context.drawString(
				MC.font, Component.literal(
					String.format(
						tr("notfirmament.config.hud.fps-count-hud.display", "FPS: %s").string, MC.instance.fps
					)
		...
		if (TConfig.pingCount) {
			...
			it.context.drawString(
				MC.font, Component.literal(
					String.format(
						tr("notfirmament.config.hud.ping-count-hud.display", "Ping: %s ms").string, ping
					)
				), 36, MC.font.lineHeight, -1, true
			)
