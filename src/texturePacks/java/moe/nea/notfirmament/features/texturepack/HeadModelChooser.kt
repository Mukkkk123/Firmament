...
object HeadModelChooser {
	...
	interface HasExplicitHeadModelMarker {
		fun markExplicitHead_NotFirmament()
		fun isExplicitHeadModel_NotFirmament(): Boolean
		...
		override fun update(
			...
			val instance =
				if (IS_CHOOSING_HEAD_MODEL.get()) {
					HasExplicitHeadModelMarker.cast(state).markExplicitHead_NotFirmament()
					head
				} else {
					regular
				}
	...
	data class Unbaked(
		...
		companion object {
			...
			fun fromLegacyJson(jsonObject: JsonObject, unbakedModel: ItemModel.Unbaked): ItemModel.Unbaked {
				val model = jsonObject["notfirmament:head_model"] ?: return unbakedModel
