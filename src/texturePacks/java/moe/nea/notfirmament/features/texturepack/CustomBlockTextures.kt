...
import moe.nea.notfirmament.features.texturepack.CustomBlockTextures.createBakedModels
import moe.nea.notfirmament.features.texturepack.CustomGlobalTextures.logger
...
import moe.nea.notfirmament.util.json.BlockPosSerializer
...
object CustomBlockTextures {
	...
	private val sodiumReloadTask = runCatching {
		val r = Class.forName("moe.nea.notfirmament.compat.sodium.SodiumChunkReloader")
			.getConstructor()
	...
	private fun prepare(manager: ResourceManager): BakedReplacements {
		...
		for ((file, resource) in resources) {
			val json =
				NotFirmament.tryDecodeJsonFromStream<CustomBlockOverride>(resource.open())
					.getOrElse { ex ->
	...
	/**
	 * Used by [moe.nea.notfirmament.init.SectionBuilderRiser]
	 */
