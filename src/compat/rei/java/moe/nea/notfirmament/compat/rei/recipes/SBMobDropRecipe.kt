package moe.nea.notfirmament.compat.rei.recipes
...
import moe.nea.notfirmament.compat.rei.EntityWidget
import moe.nea.notfirmament.compat.rei.SBItemEntryDefinition
import moe.nea.notfirmament.gui.entity.EntityRenderer
...
class SBMobDropRecipe(override val neuRecipe: NEUMobDropRecipe) : SBRecipe() {
	...
	object Category : DisplayCategory<SBMobDropRecipe> {
		override fun getCategoryIdentifier(): CategoryIdentifier<SBMobDropRecipe> =
			CategoryIdentifier.of(NotFirmament.MOD_ID, "mob_drop_recipe")
		...
		override fun setupDisplay(display: SBMobDropRecipe, bounds: Rectangle): List<Widget> {
			return buildList {
				...
				val fullMobName =
					if (level > 0) Component.translatable("notfirmament.recipe.mobs.name", level, display.neuRecipe.name)
					else Component.translatable("notfirmament.recipe.mobs.name.nolevel", display.neuRecipe.name)
				val tt = mutableListOf<Component>()
				tt.add((fullMobName))
				tt.add(Component.literal(""))
				if (display.neuRecipe.coins > 0) {
					tt.add(Component.translatable("notfirmament.recipe.mobs.coins", display.neuRecipe.coins))
				}
				if (display.neuRecipe.combatExperience > 0) {
					tt.add(
						Component.translatable(
							"notfirmament.recipe.mobs.combat",
				...
				for (drop in display.neuRecipe.drops) {
					...
					if (drop.chance != null) {
						lore += listOf(Component.translatable("notfirmament.recipe.mobs.drops", drop.chance!!))
					}
